...
global inter_nodes,global inter_voltage_sources;
global diodes;
passive =0;
voltage_sources =0;
switches =0;
diodes=0;
inter_nodes=0;
inter_voltage_sources=0;

MNA_size=0;

global MNA,global MNA_current;
global MNA_inv;
global X;
X=[];
global Z;
...
if(~isa(fid,'double') || fid<0)% file errors are not fully handled
    ...
    model_setter();
    MNA_size = passive+voltage_sources+inter_nodes+inter_voltage_sources;
    X= zeros(MNA_size,1);
    Z= zeros(MNA_size,1);
    MNA_handler_v2();
