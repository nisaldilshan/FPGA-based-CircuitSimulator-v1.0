...
function switchElement(data_arr,index)%voltage controlled switch
    %data_arr
    global switches
    switches = switches +1;
...
function diodeElement(data_arr,index)
    global diodes;
    diodes=diodes+1;
    
    global inter_nodes;
    inter_nodes = inter_nodes +1;
    
    global inter_voltage_sources;
    inter_voltage_sources=inter_voltage_sources+1;  
    
    global compNames;
    compNames{size(compNames,2)+1} = data_arr(1,:);
    global compTypes;
    compTypes{size(compTypes,2)+1} = [5;inter_nodes;inter_voltage_sources]; % for diode
    NodeFinder(2,data_arr);
    detailHandler_advanced(4,data_arr,index);
end
...
function model_extractor(data_arr)
    ...
    if(size(data_arr,1)>3)%more details available
        parameter_names = {};
        parameter_values = {};
...
function detailHandler_advanced(start,data_arr,index)
    ...
    if(isletter(data_arr(start,1)))
        ...
        obj('model_name')=model_name;% model name and extra parameter scenario not handled
